...
- **出现缺字方块（俗称豆腐块）**：模版默认使用 `fandol` 字体集以确保跨平台一致性，但其字库（GBK 标准）不全。若遇到生僻字无法显示，请在 `main.tex` 首行将 `fontset=fandol` 删去, 或修改为 `fontset=auto` 以调用系统完整字库。
   ```latex
   % 将 fandol 改为 auto 或删掉 fontset 这个参数
   \documentclass[fontset=auto]{hduthesis}
   ```
